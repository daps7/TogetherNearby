...
function setupPreferences(){
    ...
    [largeTextSetting, reducedMotionSetting, darkModeSetting].forEach(setting => {
        if(!setting) return;
...
function renderCreatedEvents(){
    ...
    list.innerHTML = events.length ? events.map(event => `
        <article class="data-item"><span class="data-item-icon">${event.emoji}</span><div><h2>${event.title}</h2><p>${event.description}</p><p class="item-detail">${event.location} · ${event.date}</p></div></article>`).join('') : '<p class="empty-state">You have not created any events yet.</p>';
}
...
function setupAccountForms(){
    ...
    if(registerForm) registerForm.addEventListener('submit', function(event){
        event.preventDefault();
        MeetTogetherDB.login(document.getElementById('registerName').value, document.getElementById('registerEmail').value);
        window.location.href = 'SplashPage.html';
    });
...
function addEvent(e){
    ...
    MeetTogetherDB.createEvent(newEvent);

    const msg = document.getElementById('addMessage');
    if(msg){
        msg.textContent = 'Event saved — returning...';
    }
